    ...
    class ScheduleActions:
        ...
        def tidyEmployeeSlots(request, schId, empId, wkmax):
            ...
            for week in sch.weeks.all():
                while week.slots.filter(employee=empl).aggregate(Sum(F('shift__hours')))['shift__hours__sum'] > int(
                        wkmax):
...
class ShiftViews:
    ...
    def coverageFormView(request, cls, sft):
        ...
        other_shifts = Shift.objects.filter(group=sft.group).exclude(pk=sft.pk).order_by(
            'name')  # type: QuerySet[Shift]
    ...
    def empShiftTallies(request, empId, doRender=True):
        ...
        value_dict = {
            "Strongly Dislike": dict(
                ...
                count=Slot.objects.filter(shift__in=emp.shift_prefs.filter(score=-2).values('shift'),
                                          employee=emp).count()),
            "Dislike": dict(
                shifts=emp.shift_prefs.filter(score=-1).values('shift__name'),
                count=Slot.objects.filter(shift__in=emp.shift_prefs.filter(score=-1).values('shift'),
                                          employee=emp).count()),
            "Neutral": dict(
                shifts=emp.shift_prefs.filter(score=0).values('shift__name'),
                count=Slot.objects.filter(shift__in=emp.shift_prefs.filter(score=0).values('shift'),
                                          employee=emp).count()),
            "Like": dict(
                shifts=emp.shift_prefs.filter(score=1).values('shift__name'),
                count=Slot.objects.filter(shift__in=emp.shift_prefs.filter(score=1).values('shift'),
