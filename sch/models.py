...
from django.db.models import (Avg, Count, ExpressionWrapper, F,
                              FloatField, OuterRef, Q, QuerySet,
                              Subquery, Sum)
...
class SlotManager(models.QuerySet):
    ...
    def belowAvg_sftPrefScore(self):
        query = self.annotate(
            score=ShiftPreference.objects.filter(employee=OuterRef('employee'), shift=OuterRef('shift')).values(
                'score'))
        ...
        query = query.annotate(
            avgScore=Subquery(
                Employee.objects.avg_shift_pref_score().filter(pk=OuterRef('employee__pk')).values('avgSftPref')))
    ...
    def unusualFills(self):
        ...
        slots = self.filter(shift=Subquery(ssts.values('shift')),
                            workday__sd_id=Subquery(ssts.values('sd_id'))).exclude(employee=None)
        for sst in ssts:
    ...
    def disliked(self):
        return self.annotate(score=
                             Subquery(ShiftPreference.objects.filter(employee=OuterRef('employee'),
                                                                     shift=OuterRef('shift')).values('score'))
                             ).filter(score__lte=0)
...
class EmployeeManager(models.QuerySet):
    ...
    def low_option_employees(self):
        return self.annotate(available_count=Count('shifts_available')
                  ).annotate(available_avg=Avg('available_count')).filter(
            available_count__lt=F('available_avg')
                )
    ...
    def fill_time_employees(self):
        return self.filter(fte__gte=0.8)

    def part_time_employees(self):
        return self.filter(fte__lt=0.8, fte__gt=0.0)
    ...
    def can_fill_shift_on_day(self, shift, dept, workday, method="available"):
        ...
        weeklyHours = {empl: Slot.objects.empls_weekly_hours(workday.date.year, workday.iweek, empl) for empl in
                       employees}
    ...
    def weekly_hours(self, year, week):
        return self.filter(slot__workday__iweek=week, slot__workday__date__year=year).aggregate(
            hours=Sum('slot__hours'))
...
class Organization(models.Model):
    ...
    def start_dates(self):
        ...
        for dept in self.departments.all():
            output.extend([(date, date.strftime("%D")) for date in dept.start_dates()])
        return output
...
class Department(models.Model):
    ...
    initial_start_date = models.DateField(default=dt.date(2022, 12, 25))
    schedule_week_length = models.IntegerField(default=6, choices=((2, 2), (4, 4), (6, 6), (8, 8), (10, 10)))

    def start_dates(self):
        return [self.initial_start_date + dt.timedelta(days=self.schedule_week_length * i * 7) for i in range(0, 30)]
...
class Employee(models.Model):
    ...
    def weekly_hours(self, year, iweek):
        return Slot.objects.filter(workday__date__year=year, workday__iweek=iweek, employee=self).aggregate(
            hours=Sum('shift__hours'))['hours']
    ...
    def period_hours(self, year, iperiod):
        return Slot.objects.filter(workday__date__year=year, workday__iperiod=iperiod, employee=self).aggregate(
    ...
    def info_printWeek(self, year, week):
        slots = list(
            Slot.objects.filter(employee=self, workday__date__year=year, workday__iweek=week).values_list('shift__name',
                                                                                                          flat=True))
...
class Workday(models.Model):
    ...
    def on_pto(self) -> QuerySet[Employee]:
    ...
    def list_unpref_slots(self):
        return self.filledSlots.annotate(
            score=Subquery(
                ShiftPreference.objects.filter(employee=OuterRef('employee'), shift=OuterRef('shift')).values('score'))
        ).filter(score__lt=0)
    ...
    def hours(self) -> EmployeeManager:
        return self.schedule.employees.all().annotate(
            hours=Subquery(self.slots.exclude(employee=None).values('shift__hours')))
    ...
    def on_deck(self):
        ...
        on_day = list(self.slots.values_list('employee', flat=True))
        not_on_deck += on_day
        print(not_on_deck)
...
class Week(models.Model):
    ...
    def overtime_hours(self):
        data = Slot.objects.filter(week=self).order_by('employee').values('employee').annotate(
            hours=Sum('shift__hours'))
        ...
        def update_hours(self, instance):
            if instance.pk:
                for employee in instance.schedule.employees.all():
                    instance.hours.update({employee.slug: sum(
                        list(instance.slots.filter(employee=employee).values_list('shift__hours', flat=True)))})
    ...
    class Actions:
        ...
        def n_fte_overages(self, instance):
            ...
            for employee in instance.schedule.employees.all():
                if employee.fte > 0:
                    diff = \
                        instance.slots.filter(employee=employee).values('shift__hours').aggregate(Sum('shift__hours'))[
                            'shift__hours__sum'] - employee.fte_14_day
                    if diff > 0:
    ...
    class Actions:
        ...
        def update_hours(self, instance):
            ...
            for empl in instance.employees.all():
                for wk in instance.weeks.all():
                    data[empl.slug] += [
                        sum(list(wk.slots.filter(employee=empl).values_list('shift__hours', flat=True)))]
            ...
            for slot in instance.slots.random_order():
                if slot.employee == None:
                    if slot.template_employee != None:
                        ...
                        if PtoRequest.objects.filter(employee=slot.template_employee,
                                                     workday=slot.workday.date).exists():
                            print('Slot Templating Skipped due to PTO Request')
                        else:
                            slot.employee = slot.template_employee
                            slot.save()
                            print(
                                f'Filled Slot {slot.workday.date.month}-{slot.workday.date.day} {slot.shift}: with {slot.employee} via Template')
        ...
        def fillSlots(self, instance, request, **kwargs):
            ...
            for i in range(700):
                ...
                slot.fillWithBestChoice()
                print(
                    f'Filled Slots {slot.workday.date.month}-{slot.workday.date.day} {slot.shift}: with {slot.employee}')
        ...
        def solveOmap(self, instance):
            ...
            for slot in empty_slots:
                ...
                if omap['can'].exists():
                    ...
                    slot.save()
                    print(
                        f'SLOT {slot} FILLED {dt.datetime.now()} VIA OPTION-MAPPING ALGORITHM -- E_WK_HRS={sum(list(slot.week.slots.filter(employee=slot.employee).values_list("shift__hours", flat=True)))} ')
                else:
        ...
        def solvePmOnly(self, instance):
            ...
            for slot in instance.slots.empty().evenings().order_by('?'):
                ...
                if choices.exists():
                    ...
                    slot.save()
                    print(
                        f'SLOT {slot} FILLED {dt.datetime.now()} VIA PM ONLY ALGORITHM -- E_WK_HRS={sum(list(slot.week.slots.filter(employee=slot.employee).values_list("shift__hours", flat=True)))} ')
                else:
        ...
        def sch_solve_with_lookbehind(self, instance):
            ...
            for slot in emptySlots:
                if slot.workday.sd_id != 0:
                    prev = slot.workday.prevWD().slots.filter(shift__start__hour__lte=slot.shift.start.hour + 1,
                                                              shift__start__hour__gte=slot.shift.start.hour - 1)
                    choices = []
                    for p in prev:
                        if p.employee != None:
                            if p.streak:
                    ...
                    if len(choices) > 0:
                        ...
                        if PtoRequest.objects.filter(employee=chosen, workday=slot.workday.date).exists() == False:
                            if TemplatedDayOff.objects.filter(employee=chosen,
                                                              sd_id=slot.workday.sd_id).exists() == False:
                                try:
        ...
        def performUnfavorableSwaps(self, instance):
            ...
            for day in days:
                ...
                for i in range(trade_length):
                    ...
                    pm_slot = pm_slots.order_by(
                        ShiftPreference.objects.filter(employee=OuterRef('pk').values('score'))).first()
...
class RoutineLog(models.Model):
    ...
    def __str__(self): return f'{self.schedule.slug} [LOG]'
...
class Slot(models.Model):
    ...
    class Actions:
        ...
        def get_conflicting(self, instance):
            ...
            if instance.actions.conflicting_slots(instance):
                return set(
                    instance.actions.conflicting_slots(instance).values_list('employee__pk', flat=True)).difference(
                    {None})
            else:
        ...
        def fills_with(self, instance):
            ...
            week_hours = Slot.objects.filter(
            ...
            pd_hours = Slot.objects.filter(
                workday__schedule=sch,
                workday__week__period=instance.workday.week.period,
                employee=OuterRef('pk')
                    ).values('employee').annotate(
                pd_hours=Sum('shift__hours')
                    ).values('pd_hours')[:1]

            return instance.shift.available.exclude(pk__in=excluding).annotate(
                week_hours=Coalesce(week_hours, 0.0),
                pd_hours=Coalesce(pd_hours, 0.0),
                    ).filter(
                week_hours__lt=F('std_wk_max')
                    ).order_by('-week_hours', 'pd_hours')

        @staticmethod
        def set_template_employee(instance, force=True):
            """Sets the template employee for a slot, and clears any conflicting slots if force=True"""
    ...
    def is_postturnaround(self):
        ...
        if self.workday.sd_id != 0:
            if self.shift.group == 'AM':
                if self.workday.prevWD().slots.filter(shift__group__in=['PM', 'XN'],
                                                      employee=self.employee).count() > 0:
                    return True
                if self.workday.sd_id != 1:
                    if self.workday.prevWD().prevWD().slots.filter(shift__group='XN',
                                                                   employee=self.employee).count() > 0:
                        return True
        return False
    ...
    def isFromTemplate(self):
        if self.employee != None:
            if ShiftTemplate.objects.filter(sd_id=self.workday.sd_id, shift=self.shift,
    ...
    def _get_conflicting_slots(self):
        ...
        if self.shift.start.hour > 10:
            ...
            slots = Slot.objects.filter(schedule=sch, workday=self.workday.nextWD(),
                                        shift__start__hour__lt=10) | Slot.objects.filter(workday=self.workday.prevWD(),
                                                                                         shift__start__hour__gt=10)
    ...
    def _save_empl_sentiment(self):
        ...
        if ShiftPreference.objects.filter(employee=self.employee, shift=self.shift):
            ...
            self.empl_sentiment = sc[
                ShiftPreference.objects.filter(employee=self.employee, shift=self.shift).first().score]
        else:
...
class ShiftTemplate(models.Model):
    ...
    employee = models.ForeignKey('Employee', on_delete=models.CASCADE, related_name='shift_templates', null=True,
                                 blank=True)
    sd_id = models.IntegerField()

    def __str__(self):
        return f'{self.shift.name} Template ' + "Sun Mon Tue Wed Thu Fri Sat".split(" ")[self.sd_id % 7] + "-" + \
            "ABCDEFG"[self.sd_id // 7]
