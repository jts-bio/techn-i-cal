...
class EmployeeTable(tables.Table):
    ...
    class Meta:
        ...
        attrs = {
            "class": "table table-auto table-striped table-md min-w-full divide-y divide-gray-200 text-sm dark:divide-gray-700"}
        sort_by = ('name',)
...
class ShiftListTable(tables.Table):
    ...
    ```
    """
    name = tables.columns.LinkColumn("sch:shift-detail", args=[A("slug")])
    hours = tables.columns.Column(verbose_name="Hours",
                                    attrs={"td": {"class": "small text-xs"}})
    percent_templated = tables.columns.Column(verbose_name="% Templated",
                                    attrs={"td": {"class": "small text-xs text-center"}})
    occur_days = tables.columns.Column(verbose_name="Scheduling Weekdays",
                                    attrs={"td": {"class": "text-center text-indigo-300"}})
    group = tables.columns.Column(verbose_name="Time-of-Day",
                                    attrs={"td": {"class": "text-center", "style": "font-family:'Helvetica Neue';"}})
    prefs = tables.columns.Column(verbose_name="Avg Explicit Preference",
                                    attrs={"td": {"class": "text-center"}})
    sort_prefs = tables.columns.Column(verbose_name="Avg Sorted Preference",
                                    attrs={"td": {"class": "text-center"}})
    ...
    class Meta:
        ...
        attrs = {
            "class": "table table-compact table-md min-w-full divide-y " \
                     "divide-gray-200 text-sm dark:divide-gray-700m-4 " \
                     "lg:m-[50px]"
        }
...
class ShiftsWorkdaySmallTable(tables.Table):
    ...
    def render_employee(self, value, record):
        ...
        elif record.department.slug == "cpht":
            return format_html(
...
class WorkdayListTable(tables.Table):
    ...
    class Meta:
        ...
        attrs = {"class": ["rounded-lg w-full text-sm text-left text-gray-500 dark:text-gray-400",
