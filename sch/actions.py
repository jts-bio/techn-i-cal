...
class WorkdayActions:
    ...
    def bulk_create(date_from: dt.date, date_to: dt.date):
        ...
        date = date_from
        while date <= date_to:
            if not Workday.objects.filter(date=date).exists():
...
class ScheduleBot:
    ...
    def is_agreeable_swap(slotA, slotB):
        ...
        if ShiftPreference.objects.filter(employee=employeeA, shift=slotB.shift).exists():
            ...
            return None
        if Slot.objects.incompatible_slots(shift=slotB.shift, workday=slotB.workday).filter(
                employee=employeeA).exists():
            return None
        ...
        if ShiftPreference.objects.filter(employee=employeeB, shift=slotA.shift).exists():
            ...
            return None
        if Slot.objects.incompatible_slots(shift=slotA.shift, workday=slotA.workday).filter(
                employee=employeeB).exists():
            return None
    ...
    def solveSchedule(self, schId):
        ...
        for empty_slot in empties:
            ...
            if len(employees) > 0:
                ...
                Slot.objects.filter(workday__sd_id=empty_slot.sd_id, employee=empl, schedule=sched).update(
                    employee=None)
                Slot.objects.filter(workday__sd_id=empty_slot.sd_id, shift=empty_slot.shift, schedule=sched).update(
                    employee=empl)
        ...
        for slot in Slot.objects.filter(is_turnaround=True, workday__iweekday__in=[1, 2, 3, 4, 5]):
            ...
            allinWeek = Slot.objects.filter(workday__date__year=slot.workday.date.year,
            ...
            for p in pos:
                if not ShiftTemplate.objects.filter(shift=p.shift, sd_id=p.workday.sd_id).exists():
                    if not Slot.objects.incompatible_slots(workday=p.workday, shift=p.shift).filter(
                            employee=slot.employee).exists():
            ...
            slot_a.employee = None;
            slot_a.save()
            slot_b.employee = None;
            slot_b.save()

            slot_a.employee = slotBEmpl;
            slot_a.save()
            slot_b.employee = slotAEmpl;
            slot_b.save()
            ...
            for ta in turnarounds:
                if ta.employee.time_pref == 'AM':
                    if Slot.objects.filter(workday__date=ta.workday.prevWD().date, employee=slot.employee,
                                           shift__start__hour__gt=10).exists():
                        Slot.objects.get(workday__date=ta.workday.prevWD().date, employee=slot.employee,
                                         shift__start__hour__gt=10).delete()
                    else:
                        ta.delete()
                else:
                    ta.delete()
...
class EmployeeBot:
    ...
    def get_emplUpcomingUnfavorables(employeeName):
        ...
        return Slot.objects.filter(workday__iweek__in=weeks, workday__date__year=td.date.year,
                                   shift__start__hour__gte=10, employee__name=employeeName)
    ...
    def empScheduleHours(employeeName, year, sch):
        ...
        return sum(list(Slot.objects.filter(
            workday__ischedule=sch, workday__date__year=year, employee__name=employeeName).values_list('shift__hours',
                                                                                                       flat=True)))
    ...
    def empScheduleHoursByWeek(employeeName, year, sch):
        ...
        for week in week_nums:
            week_totals.append(sum(list(Slot.objects.filter(
                workday__iweek=week, workday__date__year=year, employee__name=employeeName).values_list('shift__hours',
                                                                                                        flat=True))))
        return week_totals
...
class ShiftPrefBot:
    ...
    def make_avg_agg():
        ...
        return Employee.objects.annotate(avgSftPref=Avg(
            Subquery(ShiftPreference.objects.filter(employee=OuterRef('pk')).values('score'),
    ...
    def slotsBelowEmplAvg():
        ...
        return Slot.objects.annotate(
            emplAvgSftPref=Subquery(ShiftPreference.objects.filter(employee=OuterRef('employee')).values('score'),
                                    output_field=FloatField()))
...
class ExportBot:
    ...
    def scheduleForEmployee(employee):
        ...
        return wds.annotate(
            empShift=Subquery(Slot.objects.filter(employee=employee, workday=OuterRef('pk')).values('shift')))
    ...
    def predict_createdStreak(employee, workday):
        ...
        if Slot.objects.filter(employee=employee, workday=workday.prevWD()).exists():
            if Slot.objects.filter(employee=employee, workday=workday.nextWD()).exists() == False:
                ...
                i = 1
                while Slot.objects.filter(employee=employee,
                                          workday__date=workday.date + dt.timedelta(days=i)).exists():
                    i += 1
...
class GhostSlot:
    ...
    def toExcludeTurnarounds(workday, shift):
        if shift.start.hour >= 10:
            return Slot.objects.filter(workday=workday.nextWD(), shift__start__hour__lte=10).values(
                'employee__pk').distinct()
        if shift.start.hour < 10:
            return Slot.objects.filter(workday=workday.prevWD(), shift__start__hour__gte=10).values(
                'employee__pk').distinct()
