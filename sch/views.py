...
class WORKDAY:
    ...
    class WorkDayDetailView(DetailView):
        ...
        def get_context_data(self, **kwargs):
            ...
            shifts = shifts.annotate(
                ...
                prefScore=Subquery(
                    ShiftPreference.objects.filter(shift=OuterRef('pk'), employee=OuterRef('assign')).values('score'))
            )
            context['shifts'] = shifts.order_by('start')
            try:
                context['overallSftPref'] = int(
                    shifts.aggregate(Sum(F('prefScore')))['prefScore__sum'] / (2 * len(slots)) * 100)
            except:
            ...
            unfilled = Shift.objects.filter(occur_days__contains=self.object.iweekday).exclude(
                pk__in=slots.values('shift'))
            # ANNOTATION FOR # OF TECHS WHO COULD FILL SHIFTS SLOT 
            for sft in unfilled:
                sft.n_can_fill = Employee.objects.can_fill_shift_on_day(shift=sft, cls=sft.cls,
                                                                        workday=self.object).values('pk').count()
            context['unfilled'] = unfilled

            context['ptoReqs'] = PtoRequest.objects.filter(workday=self.object.date, status__in=['P', 'A']).values(
                'employee__name')
    ...
    class ResolvePtoRequestFormView(FormView):
        ...
        def get_context_data(self, **kwargs):
            context = super().get_context_data(**kwargs)
            context['slot'] = Slot.objects.get(workday__slug=self.kwargs['date'],
                                               employee__name=self.kwargs['employee'])
            context['ptoreq'] = PtoRequest.objects.get(workday=self.kwargs['date'],
                                                       employee__name=self.kwargs['employee'])
            initial = {
                'slot': context['slot'],
        ...
        def form_valid(self, form):
            slot_val = form.cleaned_data['slot']
...
class WEEK:
    class WeekView(ListView):
        ...
        def get_context_data(self, **kwargs):
            ...
            weekprefScore = sum(week.slots.exclude(employee=None).values_list('empl_sentiment', flat=True)) / len(
                week.slots.exclude(employee=None))

            for day in context['workdays']:
                day.table = self.render_day_table(day)
                prefScore = sum(day.slots.exclude(employee=None).values_list('empl_sentiment', flat=True)) / len(
                    day.slots.exclude(employee=None))
            ...
            context['turnarounds'] = (
                        Slot.objects.filter(workday__in=context['workdays'], is_turnaround=True) | Slot.objects.filter(
                    workday__in=context['workdays'], is_preturnaround=True))
        ...
        def get_day_table(self, workday):
            ...
            sftAnnot = sftAnnot.annotate(
                employeeSlug=Subquery(slots.filter(shift=OuterRef('pk')).values('employee__slug')))
        ...
        def get_hours_queryset(self, year, week):
            return Employee.objects.all().annotate(
                hours=Subquery(
                    Slot.objects.filter(workday__date__year=year, workday__iweek=week, employee=F('pk')).aggregate(
                        hours=Sum('hours')))
            )
    ...
    class GET:
        def getPercentsWorkedWeek(self, year, week):
            ...
            return [{'employeeSlug': e[2].replace(' ', '-'), 'employee': e[2], 'week_hours': e[1], 'week_percent': e[0],
                     'css': e[3]} for e in s]
        ...
        def getTurnarounds(self, year, weekId):
            slots = Slot.objects.filter(workday__date__year=year, workday__iweek=weekId,
                                        is_turnaround=True) | Slot.objects.filter(workday__date__year=year,
                                                                                  workday__iweek=weekId,
                                                                                  is_preturnaround=True)
            return slots
    ...
    def weekHoursTable(request, year, week):
        ...
        for empl in empls:
            empl.week = [
                wds.filter(iweekday=i, slot__employee=empl).order_by('iweekday').values_list('slot__shift__name',
                                                                                             flat=True) for i in
                range(7)]
    ...
    def all_weeks_view(request):
        ...
        for week in weeks:
            slotCounts.append(
                Slot.objects.filter(workday__date__year=week['date__year'], workday__iweek=week['iweek']).count())
        context = {
            ...
            while i > 0:
                ...
                if bestswap != None:
                    ...
                    messages.success(request, [
                        f"{bestswap[0].shift}] {bestswap[0].employee} swapped with [{bestswap[1].shift}] {bestswap[1].employee}"])
                else:
                    i -= 1
        for empl in Employee.objects.all():
            print(empl.info_printWeek(year, week))
    ...
    class ClearWeekSlotsView(FormView):
        ...
        def form_valid(self, form):
            if form.cleaned_data['confirm']:
                Slot.objects.filter(workday__date__year=self.kwargs['year'],
                                    workday__iweek=self.kwargs['week']).delete()
            return super().form_valid(form)
    ...
    class WeeklyUnfilledSlotsView(ListView):
        ...
        def get_context_data(self, **kwargs):
            ...
            context['unfilled_slots'] = [
                (day, shift, Employee.objects.can_fill_shift_on_day(shift=shift, workday=day).values('pk').count()) for
                day in context['workdays'] for shift in day.getshifts]
            # sort this list by the number of employees who could fill the slot
            context['unfilled_slots'].sort(key=lambda x: x[2])
            if week != 0:
                context['nextWeek'] = week + 1
                context['prevWeek'] = week - 1
            context['hrsTable'] = [(empl, Slot.objects.empls_weekly_hours(year, week, empl)) for empl in
...
class SHIFT:
    ...
    def shiftTemplateView(request, sftId):
        ...
        SHIFT TEMPLATE VIEW
        =====================================================
        Formset View for ShiftTemplates related to a Shift
        
        args:
            - sftId: Shift.pk
        """
        shift = Shift.objects.get(pk=sftId)
        html_template = 'sch/shift/shift_sst_form.html'
        SstFormSet = formset_factory(SSTForm, extra=0)

        # ---------- POST HANDLING
        if request.method == 'POST':
            formset = SstFormSet(request.POST)
            if formset.is_valid():
                formset.save()
                return reverse('sch:shift-detail', kwargs={'name': shift.name})

        # ----------- INITIAL DATA FORMATTING
        initial = [{'shift': shift, 'sd_id': i, 'employee': None} for i in range(42)]
        for ini in initial:
            if ShiftTemplate.objects.filter(sd_id=ini['sd_id'], shift=shift).exists() == True:
                ini['employee'] = ShiftTemplate.objects.get(sd_id=ini['sd_id'], shift=shift).employee

        formset = SstFormSet(initial=initial)
...
class EMPLOYEE:
    ...
    class EmployeeDetailView(DetailView):
        ...
        def get_context_data(self, **kwargs):
            ...
            ssts = []
            for i in range(42):
                if ShiftTemplate.objects.filter(employee=self.object, sd_id=i).exists():
                    ssts.append({"type": "SST",
                                 "shift": ShiftTemplate.objects.filter(employee=self.object, sd_id=i).first().shift})
                elif TemplatedDayOff.objects.filter(employee=self.object, sd_id=i).exists():
                    ssts.append({"type": "TDO", "shift": "Day Off"})
                else:
                    ssts.append({"type": "NONE", "shift": "None"})

            context['allTemplateDays'] = ssts
            initial = {
                'employee': self.object,
                'date_from': dt.date.today() - dt.timedelta(days=int(dt.date.today().strftime("%w"))),
                'date_to': dt.date.today() - dt.timedelta(days=int(dt.date.today().strftime("%w"))) + dt.timedelta(
                    days=42)
            }
    ...
    def shift_preference_form_view(request, empId):
        context = {}
    ...
    def sortShiftPreferences(request, name):
        ...
        if request.method == "POST":
            ...
            for pref in list(request.POST.values())[1:]:
                ...
                ssp = ShiftSortPreference.objects.get_or_create(employee=employee, shift=shift)
                ssp.score = i
                i += 1
                ssp.save()
    ...
    def employeeTemplatedDaysOffView(request, empId):
            ...
            if formset.is_valid():
                for form in formset:
                    if form.cleaned_data.get('is_templated_off') == False:
                        if TemplatedDayOff.objects.filter(employee=employee,
                                                          sd_id=form.cleaned_data.get('sd_id')).exists():
                        ...
                        if TemplatedDayOff.objects.filter(employee=employee,
                            ...
                            templated_day_off = TemplatedDayOff.objects.create(employee=employee,
                                                                               sd_id=form.cleaned_data.get('sd_id'))
        ...
        initial = [{'sd_id': i, 'employee': employee,
                    'is_templated_off': TemplatedDayOff.objects.filter(employee=employee, sd_id=i).exists()} for i in
                   range(42)]
    ...
    class EmployeeShiftTallyView(DetailView):
        ...
        def get_context_data(self, **kwargs):
            ...
            context['stronglyDislikeCount'] = Slot.objects.filter(employee=context['employee'],
                                                                  shift__name__in=context['stronglyDislike']).count()
            context['dislike'] = Shift.objects.filter(id__in=d).values('name')
            context['dislikeCount'] = Slot.objects.filter(employee=context['employee'],
                                                          shift__name__in=context['dislike']).count()
            context['neutral'] = Shift.objects.filter(id__in=n).values('name')
            context['neutralCount'] = Slot.objects.filter(employee=context['employee'],
                                                          shift__name__in=context['neutral']).count()
            context['like'] = Shift.objects.filter(id__in=l).values('name')
            context['likeCount'] = Slot.objects.filter(employee=context['employee'],
                                                       shift__name__in=context['like']).count()
            context['stronglyLike'] = Shift.objects.filter(id__in=sl).values('name')
            context['stronglyLikeCount'] = Slot.objects.filter(employee=context['employee'],
                                                               shift__name__in=context['stronglyLike']).count()
    ...
    def coWorkerView(request, nameA, nameB):
        ...
        return render(request, 'sch/employee/coworker.html',
                      {'days': days, 'emp1': emp1, 'emp2': emp2, 'percent': percent_worked_with})
...
class SLOT:
    ...
    def slot_admin_detail_view(request, slotId):
        ...
        images = Images.__all__
        context = {
            'slot': slot,
            'rotatingImagePath': random.choice(images),
        }
        return render(request, html_template, context)
...
class SCHEDULE:
    class FX:
        ...
        def theNickyCoefficient(self, year, sch):
            slotsTogether = Slot.objects.filter(workday__date__year=year, workday__ischedule=sch,
                                                employee__name__in=['Josh', 'Nicki'])
    ...
    def scheduleView(request, schId):
        ...
        for empl in employees:
            schedule = sched.workdays.filter(pk__in=context['days'].values('pk')).annotate(
                emplShift=Subquery(
                    Slot.objects.filter(employee=empl.pk, workday=OuterRef('pk')).values('shift__name')[:1])).annotate(
                ptoReq=Subquery(
                    PtoRequest.objects.filter(employee=empl.pk, workday=OuterRef('date')).values('employee__name')[
                    :1])).annotate(
                streakPref=Subquery(Employee.objects.filter(pk=empl.pk).values('streak_pref')[:1])).annotate(
                streak=Subquery(
                    Slot.objects.filter(employee=empl.pk, workday=OuterRef('pk')).values('streak')[:1])).values_list(
                'iweekday', 'emplShift', 'streak', 'ptoReq', 'streakPref')
    ...
    def weeklyOTView(request, year, sch):
        ...
        for emp in Employee.objects.all():
            weeklyHrs = []
            for w in weeks:
                weeklyHrs += [Slot.objects.filter(employee=emp, workday__iweek=w).aggregate(Sum('shift__hours'))[
                                  'shift__hours__sum']]
            employees += [{'name': emp.name, 'weeklyHrs': weeklyHrs}]

        return render(request, 'sch/schedule/ot.html',
                      {'employees': employees, 'weeks': weeks, 'year': year, 'sch': sch})
    ...
    def breakupLongStreaks(request, year, sch):
        ...
        nUnfilled_initial = sum(
            [wd.emptySlots.count() for wd in Workday.objects.filter(date__year=year, ischedule=sch)])
...
class TEST:
    ...
    def possibleInterWeekSlotSwaps(request, workday, shift):
        ...
        for possibleSlot in possible:
            ...
            if possibleSlot.employee.shifts_trained.contains(slot.shift):
                ...
                if Slot.objects.incompatible_slots(workday=slot.workday, shift=slot.shift).filter(
                        employee=possibleSlot.employee).exists():
                    pass
                else:
    ...
    def allOkIntraWeekSwaps(request, workday, shift):
        ...
        empA_weekslots = Slot.objects.filter(employee=empA, workday__iweek=slot.workday.iweek).exclude(
