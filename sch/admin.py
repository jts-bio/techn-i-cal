...
class DeptShiftsInline(admin.TabularInline):
    model = Shift
    fields = ['name', 'start', 'duration', 'hours', 'department', 'phase', 'occur_days']
...
class DeptEmployeesInline(admin.StackedInline):
    model = Employee
    fields = ['name','department',
              'initials','fte',
              'time_pref', 'streak_pref',
    ...
    def get_formset(self, request, obj=None, **kwargs):
        formset = super(DeptEmployeesInline, self).get_formset(request, obj, **kwargs)
        formset.form.base_fields['time_pref'].widget = forms.Select(
...
class OrganizationDepartments(admin.StackedInline):
    model = Department
    fields = ['name', 'slug', 'org', 'initial_start_date', 'schedule_week_length']
    readonly_fields = ['org']
    extra = 0
    can_delete = True
...
class DepartmentAdmin(admin.ModelAdmin):
    ...
    inlines = [DeptShiftsInline, DeptScheduleTable, DeptEmployeesInline]
...
class EmployeeAdmin(admin.ModelAdmin):
    ...
    list_display = ['name', 'initials', 'fte', 'streak_pref', 'time_pref']
...
class SlotAdmin(admin.ModelAdmin):
    fields = ['workday', 'shift', 'employee', '  start', 'slug', 'fill_candidates']
    readonly_fields = ['start', 'slug', 'fill_candidates', 'streak']
